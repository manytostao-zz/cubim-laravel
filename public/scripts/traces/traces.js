...
var traces = function () {
    ...
    var _initFilterButtons = function () {
        var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
        $('input[type=radio][name=options]').on('change', function (e) {
            e.preventDefault();
            var form = {};
            var date = utilities.getCubanFormattedDate();
            if (this.value === 'today') {
                form['from_creation_date'] = date;
                form['to_creation_date'] = date;
            }
            if (this.value === 'week') {
                var from = utilities.getCubanFormattedDate(-7);
                form['from_creation_date'] = from;
                form['to_creation_date'] = date;
            }
            if (this.value === 'month') {
                var from = utilities.getCubanFormattedDate(0, -1);
                form['from_creation_date'] = from;
                form['to_creation_date'] = date;
            }
            form['_token'] = $('#form__token').val();
            $.ajax({
                url: laroute.route('traces.filter'),
                type: 'POST',
                data: {_token: CSRF_TOKEN, form: form},
                success: function (data) {
                    $('#tracesListDatatable').DataTable().ajax.reload();
                },
                error: function (data) {
                    alert(data);
                }
