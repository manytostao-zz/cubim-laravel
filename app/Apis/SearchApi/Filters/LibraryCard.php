...
class LibraryCard implements Filter
{
    /**
    ...
    public static function applyWhere(Builder $builder, $value)
    {
        if (!is_null($value) and $value != '')
            return $builder->where('library_card', 'like', '%' . $value . '%');
        return $builder;
    }
    ...
    {
        return $builder->orderBy('library_card', $dir);
    }
    ...
    {
        if (!is_null($value) and $value != '')
            return $builder->orWhere('library_card', 'like', '%' . $value . '%');
        return $builder;
    }
}